...
def import_psa(context: Context, psa_reader: PsaReader, armature_object: Object, options: PsaImportOptions) -> PsaImportResult:
    ...
    for psa_bone_index, psa_bone in enumerate(psa_reader.bones):
        ...
        if armature_bone_index is not None:
            # Only map the first PSA bone to a given armature bone — ignore duplicates silently.
    ...
    # Log which bones were successfully matched.
    matched_bone_names = sorted(set(psa_bone_names) & set(b.name for b in armature_data.bones))
    total_psa_bones = len(psa_reader.bones)
    result.infos.append(f'{len(matched_bone_names)}/{total_psa_bones} PSA bones matched to armature \'{armature_object.name}\'')
    if matched_bone_names:
        result.infos.append(f'Matched bones: {", ".join(matched_bone_names)}')
    ...
    for (psa_bone_index, psa_bone), psa_bone_name in zip(enumerate(psa_reader.bones), psa_bone_names):
        ...
        # Use .get() for safe bone lookup — if the armature bone name is missing
        # (e.g. due to a rename or data inconsistency), skip instead of crashing.
        armature_bone = armature_data.bones.get(psa_bone_name)
        pose_bone = armature_object.pose.bones.get(psa_bone_name)
        if armature_bone is None or pose_bone is None:
            # Bone was mapped by regex but name lookup failed — skip silently.
            del psa_to_armature_bone_indices[psa_bone_index]
            import_bones.append(None)
            continue
        import_bone.armature_bone = armature_bone
        import_bone.pose_bone = pose_bone
    ...
    for import_bone in filter(lambda x: x is not None, import_bones):
        ...
        if has_parent:
            parent_name = armature_bone.parent.name
            if parent_name in psa_bone_names:
                # Use .get() for safe lookup — the parent bone name may be in
                # psa_bone_names but not in psa_bone_names_to_import_bones if it
                # was a duplicate mapping or was skipped for another reason.
                parent_import_bone = psa_bone_names_to_import_bones.get(parent_name)
                if parent_import_bone is not None:
                    import_bone.parent = parent_import_bone
                # If parent not found in import dict, leave parent as None and proceed silently.
        # Calculate the original location & rotation of each bone.
        if has_parent:
    ...
    for sequence_index, sequence in enumerate(sequences):
        ...
        if options.should_write_keyframes:            
            ...
            for psa_bone_index, armature_bone_index in psa_to_armature_bone_indices.items():
                ...
                if import_bone is None:
                    continue  # Bone was skipped (e.g. armature bone not found), skip its fcurves.
        ...
        result.infos.append(f'Sequence \'{sequence_name}\': {len(psa_to_armature_bone_indices)} bones, {sequence.frame_count} frames @ {target_fps:.1f} fps → \'{action_name}\'')

        context.window_manager.progress_update(sequence_index)

    # Assign the last imported action as the active action on the armature
    # so the user sees the animation immediately.
    if actions:
        if armature_object.animation_data is None:
            armature_object.animation_data_create()
        armature_object.animation_data.action = actions[-1]
        result.infos.append(f'Assigned \'{actions[-1].name}\' as active action on \'{armature_object.name}\'')
